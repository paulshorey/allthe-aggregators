...
class ModalCountryChart extends Component {
  ...
  componentDidUpdate(){
    if (Object.keys(this.props.data).length) {
      // plot
      createImportExportPlot(this.props.data, this.props.data.country_name, document.getElementById('countryMetrics_plot'), {});

      // open modal
      $(document.getElementById('countryMetrics_modal')).modal('show');

      // self-destruct! (delete data after done with it, so this only opens once)
      this.props.dispatch(uiActionCreators.countryMetrics(false));
    }
  }
  render() {
    return (
      <div className="modal fade" id="countryMetrics_modal" role="dialog" ref={this.modalElement}>
        <div className="modal-dialog">
          {/* Modal content*/}
          <div className="modal-content">
            {/* <div className="modal-header"> */}
            {/*   <button type="button" className="close" data-dismiss="modal">&times;</button> */}
            {/*   <h4 className="modal-title">modal-title ... {this.props.data.country_name}</h4> */}
            {/* </div> */}
            <div className="modal-body">
              
              {/* Main Content */}
              <div id="countryMetrics_plot"></div>
...
const mapStateToProps = state => ({
  data: state.ui.countryMetrics_data
});
